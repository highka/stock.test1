...
# --- 1. 網頁設定 ---
# 更新邏輯：修正 Matplotlib 缺失造成的 ImportError，版本升級至 1.5
# 修改內部顯示邏輯，增加防呆，後綴標註 a
VER = "ver 1.5a (Fix ImportError + Industry + Image)"
...
def run_strategy_backtest(stock_dict, progress_bar, mode, use_trend_up, use_treasure, use_vol, use_royal, use_leg_kick, min_vol_threshold):
    ...
    for i, batch_idx in enumerate(range(0, len(all_tickers), BATCH_SIZE)):
        ...
        try:
            ...
            scan_window = df_c.index[-60:] # 縮短回測範圍提升速度
            for ticker in df_c.columns:
                try:
                    c_series = df_c[ticker].dropna()
                    if len(c_series) < 100: continue
                    ...
                    for date in scan_window:
                        ...
                        if use_leg_kick:
                            ...
                            m200 = c_series.rolling(200).mean().iloc[idx] if idx >= 200 else 0
                            if c_series.iloc[idx] > m30 > m60 and (m60 > m200 or m200 == 0): is_match = True
                        
                        if is_match:
                            results.append({
                                "月份": date.strftime("%m月"), "代號": ticker.split('.')[0], "名稱": stock_info.get("name"),
                                "產業": stock_info.get("group", "其他"), "訊號日期": date.strftime("%Y-%m-%d"),
                                "訊號價": round(c_series.iloc[idx], 2), "結果": "已偵測"
                            })
                            break # 同一檔只取最新一次
                except: continue
        except: pass
            ...
            for ticker in df_c.columns:
                try:
                    ...
                    raw_data.append({
                        ...
                        "打腳發動": leg_kick, "皇冠特選": (p > df_c[ticker].rolling(30).mean().iloc[-1] > df_c[ticker].rolling(60).mean().iloc[-1])
                    })
                except: continue
        except: pass
        progress_bar.progress((i+1)/total_batches)
...
with st.sidebar:
    st.header("功能選單")
    CACHE_FILE = "stock_cache_v15.csv"
    ...
    if st.button("🧪 執行回測"):
        stock_dict = get_stock_list()
        pb = st.progress(0)
        bt_res = run_strategy_backtest(stock_dict, pb, strategy_mode, False, False, False, (strategy_mode=="🐎 多頭馬車"), (strategy_mode=="🦵 打腳發動"), min_vol)
        st.session_state["backtest_result"] = bt_res

# 主畫面
if st.session_state["master_df"] is None:
    st.warning("👈 請先點擊左側下載資料！")
    col1, col2, col3 = st.columns([1, 3, 1])
    with col2:
        if os.path.exists("welcome.jpg"):
            st.image("welcome.jpg", use_container_width=True)
            st.markdown("<p style='text-align:center'>預祝心想事成，紫氣東來! 🟣✨</p>", unsafe_allow_html=True)
else:
    df = st.session_state["master_df"].copy()
    df = df[df["成交量"] >= min_vol*1000]
    
    if strategy_mode == "🦵 打腳發動": df = df[df["打腳發動"]==True]
    elif strategy_mode == "🐎 多頭馬車": df = df[df["皇冠特選"]==True]
    
    st.subheader(f"🔍 篩選結果: {len(df)} 檔")
    if not df.empty:
        df["成交量(張)"] = (df["成交量"]/1000).astype(int)
        show_cols = ["代號", "名稱", "產業", "收盤價", "生命線", "乖離率(%)", "成交量(張)"]
        
        # --- 核心修正：加入 Try-Except 防止 matplotlib 缺失報錯 ---
        try:
            st.dataframe(df[show_cols].style.background_gradient(subset=["乖離率(%)"], cmap="RdYlGn_r"), use_container_width=True, hide_index=True)
        except ImportError:
            st.info("💡 系統提示：環境缺少 Matplotlib，暫不顯示表格背景漸層色。")
            st.dataframe(df[show_cols], use_container_width=True, hide_index=True)
            
        sel = st.selectbox("個股分析", df["代號"].tolist())
        row = df[df["代號"]==sel].iloc[0]
        fig = go.Figure(data=[go.Candlestick(x=[datetime.now()], open=[row['收盤價']], high=[row['收盤價']], low=[row['收盤價']], close=[row['收盤價']])])
        fig.update_layout(title=f"{row['名稱']} 即時位置", height=300)
